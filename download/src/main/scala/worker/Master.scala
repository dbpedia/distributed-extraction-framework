...
class Master(workTimeout: FiniteDuration) extends Actor with ActorLogging {
  ...
  def receive = {
    case ProgressReport(workerId: String, progress: Int) =>
      log.info("Heard from worker {}: {} ", workerId, progress)
      workers.get(workerId) match {
        case Some(s @ WorkerState(_, Busy(work, deadline))) =>
          workers -= workerId
            workers += (workerId -> WorkerState(sender, status = Busy(work, Deadline.now + workTimeout)))
